...
**要求**：
1. **拆解内容**：提炼本章发生的剧情，对环境、心理、外貌、语言描写进行概括。
2. **人物分析**：
   - 列出本章出现的所有人物。
   - 分析每个人的性格特征（基于行为、对话等）。
   - 描述人物之间的关系（明确或推测）。
...
`;
    ...
    const completion = await dp.chatCompletions({
      system: '你是一名专业的小说分析师，擅长从读者视角拆解和分析小说内容。',
      query: prompt,
    });
    ...
    return {
      ...
      analysis: completion.answer.trim(),
      status: 'success'
    };
  } catch (error) {
...
// 主函数
async function readBook(name, batchSize = 2, mergeSize = 30) {
  // 要处理的文件路径
